...
import edu.ucla.nesl.sigma.P.SSocketDataReceived;
import edu.ucla.nesl.sigma.P.STransactionRequest;
import edu.ucla.nesl.sigma.P.STransactionResponse;
...
import edu.ucla.nesl.sigma.api.RequestSender;
...
public class SigmaEngine {
  ...
  public SigmaEngine(Context context, URI baseURI, IRequestSender requestSender) {
    mContext = context;
    mBaseURI = baseURI;
    mSender = new RequestSender(requestSender);
  ...
  public ISigmaManager getRemoteManager(URI targetBaseURI) {
    SRequest request = (new SRequest.Builder())
        ...
        .build();
    return ISigmaManager.Stub.asInterface(proxyBinder(mSender.send(request).uri));
  }
  ...
  private void binderClose(BinderRecord record) {
    ...
    for (URI remote : record.connectedProxies) {
      ...
      SRequest request = (new SRequest.Builder())
          ...
          .target(target)
          .build();
      SResponse unusedResponse = mSender.send(request);
    }

    // !!Force garbage collection.
    // Useful to immediately visualize effect with a command like:
    // watch -n1 adb -d shell dumpsys meminfo .SigmaFactoryService{A|B}
    System.gc();
  }
  ...
  public SResponse handleRequest(SRequest request) {
  ...
  public boolean handleProxyAttached(URI source, URI target) {
    ...
    if (source.type != URI.ObjectType.BASE) {
      throwUnexpected(
          new IllegalStateException("Expecting source of proxy connection to be BASE URI"));
    }
      ...
      try {
        ...
        if (mRemoteTarget != null) {
          ...
          SRequest request = (new SRequest.Builder())
              ...
              .build();
          SResponse response = mSender.send(request);
  ...
  private class Forwarder extends Thread {
    ...
    public Forwarder(URI target, ParcelFileDescriptor parcelFd) {
      LogInstanceDebug(TAG, "created Forwarder for fd=" + parcelFd.getFd() + " --> target=" + target
          .toString());
      mParcelFd = parcelFd;
      mTarget = target;
    }
  ...
  public static IBinder getActivityManager() {
    try {
      Object activityManager = Class.forName("android.app.ActivityManagerNative")
          ...
          .invoke(null);
      return (IBinder) activityManager.getClass().getMethod("getSelfService")
