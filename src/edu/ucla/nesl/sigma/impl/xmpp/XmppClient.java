...
public class XmppClient implements ChatManagerListener {
  ...
  private class TransactionHandlerThread extends Thread {
    ...
    private class TransactionHandler extends BaseTransaction {
      ...
      public void processTransact() {
        ...
        byte[] responseBytes;
        try {
          responseBytes = mRequestHandler.handleRequest(request.toByteArray());
        } catch (RemoteException ex) {
          throwUnexpected(ex);
          return;
        }
        SResponse response = SigmaWire.getInstance().parseFrom(responseBytes, SResponse.class);
  ...
  final IRequestHandler mRequestHandler;

  public XmppClient(XmppConfig config, IRequestHandler handler) {
    mConfig = config;
