...
public class PingPongActivity extends BunchOfButtonsActivity {
  ...
  public void testNative() {
    ...
    for (int ii = 0; ii < NUM_TEST_REPS; ++ii) {
      ...
      SigmaManager sigmaA = connA.newInstance(SigmaFactoryA.getURINative(), null);
      SigmaManager sigmaB = connB.newInstance(SigmaFactoryB.getURINative(), null);
  ...
  public void testLocal() {
    ...
    for (int ii = 0; ii < NUM_TEST_REPS; ++ii) {
      boolean lastRep = (ii == NUM_TEST_REPS - 1);
      SigmaManager sigmaA = connA.newInstance(SigmaFactoryA.getURILocal(), null);
      SigmaManager sigmaB = connB.newInstance(SigmaFactoryB.getURILocal(), null);
  ...
  public void testHttp() {
    ...
    for (int ii = 0; ii < NUM_TEST_REPS; ++ii) {
      boolean lastRep = (ii == NUM_TEST_REPS - 1);
      SigmaManager sigmaA = connA.newInstance(SigmaFactoryA.getLocalHttp(), null);
      SigmaManager sigmaB = connB.newInstance(SigmaFactoryB.getLocalHttp(), null);
  ...
  public void testXmpp() {
    ...
    for (int ii = 0; ii < numReps; ++ii) {
      ...
      SigmaManager sigmaA = connA.newInstance(TestXmpp.getXmppA(), TestXmpp.getPasswordBundleA());
      SigmaManager sigmaB = connB.newInstance(TestXmpp.getXmppB(), TestXmpp.getPasswordBundeB());
      runTest(sigmaA, sigmaB, "xmpp", (ii < 1), RUN_PROFILER_ON_LAST_REP && lastRep);
    }
  }

  public void runTest(SigmaManager sigmaA, SigmaManager sigmaB, String name, boolean warmUpRun,
                      boolean withProfiler) {
