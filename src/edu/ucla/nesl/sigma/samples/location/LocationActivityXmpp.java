...
public class LocationActivityXmpp extends BunchOfButtonsActivity {
    SigmaServiceConnection sigmaConn;
    SigmaManager sigmaMan;


    @Override
    public void onCreateHook() {
        sigmaConn = new SigmaServiceConnection(this, SigmaServiceA.class);

        addButton("Login XMPP as ΣA", new Runnable() {
            @Override
            public void run() {
                sigmaMan = sigmaConn.getImpl(TestXmpp.getXmppA(), TestXmpp.getPasswordBundleA());
            }
        });
        ...
        addButton("Login XMPP as ΣB", new Runnable() {
            @Override
            public void run() {
                sigmaMan = sigmaConn.getImpl(TestXmpp.getXmppB(), TestXmpp.getPasswordBundeB());
            }
        });
    ...
    void requestLocationUpdates() {
        LocationManager nativeLocationManager = (LocationManager) getSystemService(Context.LOCATION_SERVICE);
        ...
        LocationManager remoteLocationManager = (LocationManager) remoteContext.getSystemService(Context.LOCATION_SERVICE);
